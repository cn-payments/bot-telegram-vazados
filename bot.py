...
async def database(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🔍 Comando /database recebido! Verificando permissões...")
    ...
    await update.message.reply_text("✅ Permissão confirmada! Iniciando backup...")
    ...
    try:
        ...
        logger.info(f"✅ Estrutura do banco enviada para admin {update.effective_user.id}")
        
    except Exception as e:
        logger.error(f"❌ Erro no comando /database: {e}")
        await update.message.reply_text(f"❌ Erro ao gerar estrutura do banco: {str(e)}")
